...
#------------------------------------------------------------------------#
# fixation_file_name = './assets/VQA-MHUG/fixation_csv/question-fixation-vqa-mhug-mean-correct.csv'
fixation_file_name = './assets/VQA-MHUG/fixation_csv/SAMPLE-question-fixation-vqa-mhug-mean-correct.csv'
qa_file_name = './assets/VQA-MHUG/VQA-v2/questions/v2_mscoco_val2014_refined_qa.json'
##########################################################################
##########################################################################
##########################################################################
...
for question_fixation in fixation_file:
    ...
    image = Image.open("./assets/VQA-MHUG/VQA-v2/images/COCO_val2014_"+str(image_id).zfill(12)+".jpg")
    ...
    answers[key]['answer_set'] = qa[question_fixation[0]]['answer_set']
    ...
    answer_count = 0
    for answer_candidate in qa[question_fixation[0]]['answer_set']: 
        if answer_candidate.lower() in output_text.lower():
            answer_count = answer_count + 1
    answers[key]['correct'] = min(answer_count/3, 1)
    correct = correct + min(answer_count/3, 1)
    wrong = wrong + (1 - min(answer_count/3, 1))
...
# outfile_name = './results_vqamhug/answers_'\
#                 +model_name.split('/')[1]\
